...
int main() {
  ...
  auto sin_init = [=] __cuda_callable__(
    ...
    u_view[i.y() + i.x() * ref.numDOF()] =
        -TNL::sin((xL + (r + 1.0) * 0.5 * h) * PI) + 1.f;
  };

  auto shock_init =
      [=] __cuda_callable__(
          const TNL::Containers::StaticArray<2, int> &idx) mutable {
        if (idx.x() < int(mesh.numElements() / 3) ||
            idx.x() > int(mesh.numElements() / 2)) {
  ...
  auto saw_init = [=] __cuda_callable__(
                      const TNL::Containers::StaticArray<2, int> &idx) mutable {
    if (idx.x() < int(mesh.numElements() / 3) ||
        idx.x() > int(2 * mesh.numElements() / 3)) {
      u_view[idx.y() + idx.x() * ref.numDOF()] = 0.0;
    } else {
      u_view[idx.y() + idx.x() * ref.numDOF()] = 1.0;
    }
  ...
  auto cone_init =
      [=] __cuda_callable__(
        ...
        if (idx.x() < int(mesh.numElements() / 4)) {
          u_view[idx.y() + idx.x() * ref.numDOF()] =
              (xL + (r + 1.0) * 0.5 * h) / (h * int(mesh.numElements() / 4)) +
              2.0;
        } else if (idx.x() >= int(mesh.numElements() / 4) &&
                   idx.x() < int(mesh.numElements() / 2)) {
          u_view[idx.y() + idx.x() * ref.numDOF()] =
              -(xL + (r + 1.0) * 0.5 * h) / (h * int(mesh.numElements() / 4));
        } else {
  ...
  for (int i = 0; i < mesh.numElements(); i++) {
    for (int j = 0; j < ref.numDOF(); j++) {
      max_speed =
          TNL::argAbsMax(max_speed, advection_speed(u.elementPtr(i)[j]));
    }
  }
  ...
  auto callback = [&](Real t, const DG::FieldVector<Real> &uh,
                      int step) -> bool {
  ...
  int pade_frame = 0;
  DG::writePadeTimeSeriesVTK(mesh, ref, u, pade_solver, "output/pade_output",
                             pade_frame++, Tf);
